 ...
 */
 ...
 import type {
  ...
  /**
   * Step 5 — "Difficulty", plus the target contributor `role` this task is
   * curated for. The spec only names Difficulty explicitly, but the
   * DevTunnel home feed already models a task's `role` (see
   * `lib/home/types.ts`'s `RecommendedTask.role`, e.g. "Backend
   * Developer") — so `role` here reuses the same `DeveloperRole` enum the
   * contributor onboarding flow already defines
   * (`lib/onboarding/types.ts`) rather than inventing a second taxonomy
   * for what is the same underlying concept.
   */
