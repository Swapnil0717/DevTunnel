import type {
  ...
  /**
   * Step 5 of Project Onboarding — "Final Validation / Confirmation"
   * (admin_workflow.txt, "Step 5 — Final Validation / Confirmation" and
   * section 7, "Project Creation Backend").
   *
   * The checklist reflects `draft.steps` exactly as returned by the
   * backend — never a locally-computed guess (section 24: "Do not let the
   * frontend determine whether onboarding is complete. Backend should
   * maintain the state."). "Create / Import Project" calls
   * `POST .../:id/complete`, which the backend is specified to reject
   * outright if any step isn't actually done ("Everything valid? NO →
   * Reject") — this component surfaces that rejection as `createError`
   * rather than assuming the click always succeeds.
   *
   * Per section 6: "The project does not exist as an active DevTunnel
   * project before this point." — so until `createdProject` is set, nothing
   * on this screen implies the project is live yet.
   */
