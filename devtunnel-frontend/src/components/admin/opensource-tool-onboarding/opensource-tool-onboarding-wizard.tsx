 ...
 *
 * Built incrementally, one step at a time: only Step 1 (Tool URL) is
 * wired to real state and a real API call so far. Steps 2–4 render an
 * honest `SectionMessage` placeholder — same "don't fake functionality
 * that doesn't work yet" convention the rest of the Admin Portal uses
 * (Frontend_Development_Rules.txt rule 26) — until each is built and
 * swapped in.
 */
export function OpenSourceToolOnboardingWizard() {
  ...
  const canContinue = step === 1 ? Boolean(draft?.source) : true;
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-6 sm:px-6 sm:py-10 lg:px-16 lg:py-14 xl:px-20">
        <div className="flex w-full max-w-[720px] flex-col gap-6 sm:gap-8">
          <div className="flex-1">
            {step === 1 && <ToolUrlStep draft={draft} onImported={setDraft} />}
            ...
            {step === 2 && (
              <div>
                ...
                <p className="m-0 mb-6 max-w-[520px] text-[13px] leading-[1.6] text-text-muted">
                  Choose whether DevTunnel uses the fetched description as-is
                  or a custom one.
                </p>
