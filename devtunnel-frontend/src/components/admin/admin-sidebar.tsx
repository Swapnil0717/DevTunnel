...
import { ADMIN_NAV_ITEMS, type AdminNavLink } from "./admin-nav-items";

/**
 * Left app-shell navigation for the Admin Portal (Admin Portal Master
 * Coding Specification, section 2 — Admin Navigation), visible from `md`
 * and up. `AdminMobileNav` is the narrow-viewport counterpart, same
 * convention as `AppSidebar` / `AppBottomNav` on the contributor side.
 *
 ...
 *
 * Renders `ADMIN_NAV_ITEMS` as given: a top-level link (Dashboard), two
 * groups with their own indented sub-links (Projects, Tasks), and a
 * trailing top-level link (Activity) — the exact tree the spec draws.
...
export function AdminSidebar() {
  ...
  const renderLink = (link: AdminNavLink) => {
    const { href, label, Icon, built } = link;
      ...
      <nav aria-label="Admin" className="flex-1">
        <ul className="flex flex-col gap-1 list-none p-0 m-0">
          {ADMIN_NAV_ITEMS.map((entry) => {
            ...
            return (
              <li key={label} className="mt-2 first:mt-0">
                <p className="flex items-center gap-3 px-3 py-1.5 text-[11px] font-medium uppercase tracking-wide text-text-faint">
                  <Icon className="h-[15px] w-[15px] shrink-0" />
                  {label}
                </p>
                <ul className="flex flex-col gap-1 list-none p-0 m-0 pl-[14px]">
