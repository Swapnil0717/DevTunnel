...
export function LoadingPanel({
  ...
  label?: string;
  /**
   * Optional secondary line under `label` — smaller and dimmer, for
   * context that reassures the user a slow wait is expected rather
   * than stuck (e.g. "this can take a minute"). Omit for panels
   * where the wait is short enough not to need reassurance.
   */
  description?: string;
  className?: string;
}) {
  return (
    ...
    >
      ...
      {description ? (
        <p className="m-0 max-w-[420px] text-center text-[11.5px] text-text-faint">
          {description}
        </p>
      ) : null}
    </div>
