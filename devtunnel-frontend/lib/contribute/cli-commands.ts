 ...
 * routes behind it (`/auth/cli/*`, `/projects/:id/start`,
 ...
 *
 *  - Contributing to the project itself, with no specific task in mind,
 *    needs only the project's own id — its UUID, NOT its slug: the
 *    backend's `/projects/:id/start` and `/projects/:id/submit` reject
 *    anything that isn't a UUID with "Invalid project id", so a slug in
 *    this command fails for every contributor who pastes it. It's passed
 ...
 * no project id (it isn't a row in `devtunnel.projects`, and the backend
 * has no tool-claim route), so with `projectId: null` the project-scoped
 * `dev start`/`dev submit` rows are left out entirely rather than shown
 * with an id that would be rejected.
 *
...
export interface CliCommandsInput {
  /** The project's DevTunnel id (UUID) — what `dev start`/`dev submit --project` take when there's no specific task to scope to. `null` for a tool, which has none. */
...
export function buildCliCommands({ projectId, tasks }: CliCommandsInput): CliCommand[] {
  ...
  if (projectId) {
    commands.push({
      id: "start-project",
      command: `dev start ${projectId} --project`,
