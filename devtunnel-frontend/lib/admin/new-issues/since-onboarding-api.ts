 ...
 * same, now-fully-walked `GET /admin/new-issues` list `getAdminNewIssues`
 * already fetches (see that file's note on the route not being built
 * yet), narrowed by `filterNewIssuesSinceOnboarding`. A second network
 * call for a subset of data the frontend already has would be
