-- DevTunnel — AI Discovery authored fields.
--
-- 020_add_ai_discovery.sql captured only what GitHub's API verifies
-- (stars, forks, issue numbers, license...). This migration adds the
-- fields Gemini itself writes from real source material (README, issue
-- body) — the same fields a manual onboarding wizard would ask an admin
-- to type in. Verifiable facts and authored fields are kept in separate
-- columns on purpose, so it's always possible to tell which is which:
--   - github_description / readme / stars / forks / ...  -> GitHub API, never Gemini's guess
--   - description / category / setup_guide / task_summary -> Gemini-authored
--   - difficulty / suggested_roles / suggested_difficulty  -> Gemini's pick, but ONLY from existing enum values
--
-- Run once, after 020_add_ai_discovery.sql.

alter table devtunnel.ai_discovered_projects
  add column if not exists description text not null default '',
  add column if not exists category text not null default '';

alter table devtunnel.ai_discovered_tools
  add column if not exists description text not null default '',
  add column if not exists setup_guide text not null default '';

alter table devtunnel.ai_discovered_tasks
  add column if not exists task_summary text not null default '';

-- devtunnel.projects needs somewhere real to receive `category` once a
-- candidate is approved. devtunnel.opensource_tools already has
-- setup_guide / description_source / custom_description from sql/017 —
-- no schema change needed there, approve_ai_discovered_tool below just
-- starts actually using them instead of a placeholder string.
alter table devtunnel.projects
  add column if not exists category text;

-- ---------------------------------------------------------------------------
-- approve_ai_discovered_project — now writes the Gemini-authored
-- description as a real CUSTOM description (previously it silently
-- duplicated github_description into `description` and never touched
-- `custom_description` at all — a bug this corrects), plus the new
-- `category` column.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- approve_ai_discovered_tool — now writes the real Gemini-authored
-- description and setup guide instead of the old
-- "Category: X\n\n{ai_reasoning}" placeholder.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- approve_ai_discovered_task — now uses the Gemini-authored task_summary
-- as the task's public custom_description, keeping ai_reasoning purely
-- as the admin-facing "why this is a good pick" justification.
-- ---------------------------------------------------------------------------
